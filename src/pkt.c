    ...
    while (g_keep_running) {
        ...
        if (wl3 != NULL) {
            ...
            if (pkt) {
                ...
                /* --- [3] GPS 정보는 직접 참조(Reference)로 주입 --- */
                // 위도/경도/고도는 WL-3에 없으므로 g_driving_status에서 가져옴.
...
void *sub_thread_pkt_rx(void *arg) {
    ...
    /*while (g_keep_running) {
        ...
        DBG_INFO("PKT-RX: Incoming Packet (Sender: 0x%lX)", rx_pkt->sender.sender_id);
        wl1_packet_t *to_val = malloc(sizeof(wl1_packet_t));
        if (to_val) {
            memcpy(to_val, rx_pkt, sizeof(wl1_packet_t));
