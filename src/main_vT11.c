...
// 1. RX 파이프라인 (T1 → 필터 → T2 ~ T4)
    ...
    // 2. 주행 데이터 통합 관리 (T5)
    pthread_create(&ths[5], NULL, thread_driving_manager, NULL); // T5: Driving Manager

    // 3. TX 파이프라인 (T6 ~ T8)
    pthread_create(&ths[6], NULL, sub_thread_pkt_tx, NULL);   // T6: Packet TX
    pthread_create(&ths[7], NULL, thread_sec_tx, NULL);      // T7: Security TX
    pthread_create(&ths[8], NULL, thread_tx, NULL);          // T8: Wireless TX

    // 4. 외부 센서 인터페이스 (T9)
    pthread_create(&ths[9], NULL, thread_yocto_if, NULL);    // T9: Yocto I2C
    pthread_create(&ths[10], NULL, thread_gps, NULL);
